    ...
    {{--</ul>--}}
    <p>&copy; <span class="date">{{ date("Y") }}</span> {{ trans('messages.sitename') }}. {{ trans('messages.allrightsreserved') }} <a href="http://anveto.com">{{ trans('messages.hostingby') }}</a> <a href="http://markustenghamn.com">{{ trans('messages.createdby') }}</a> <a href="/privacy-policy">{{ trans('messages.privacypolicy') }}</a> <a href="/terms-of-service">{{ trans('messages.tos') }}</a></p>